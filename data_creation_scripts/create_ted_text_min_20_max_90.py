...
- Write a single mapping file that groups sequences by S20 clusters: <basename>_min0.2.mapping.
...
def write_rows_to_text_s20(df: pd.DataFrame, output_prefix: str) -> None:
    ...
    mapping_path = os.path.join(os.path.dirname(seq_path), f"{basename_prefix}_min0.2.mapping")
...
if __name__ == "__main__":
    parquet_path_pattern = "../data/ted/s100_parquets/train_test_split_v2/train_filtered/*.parquet"
    output_dir = "../data/ted/s100_text_min_20_max_90/train_test_split_v2/train_filtered"

    parquet_files = sorted(glob.glob(parquet_path_pattern))
    print(f"Found {len(parquet_files)} parquet files matching pattern")
